...
export class PhysicVehicle extends PhysicBody {
    ...
    private readonly geometry: Ammo.btBoxShape;

    private readonly body: Ammo.btRigidBody;

    private readonly transform: Ammo.btTransform;

    private readonly motionState: Ammo.btDefaultMotionState;

    private readonly localInertia: Ammo.btVector3;

    private readonly tuning: Ammo.btVehicleTuning;

    private readonly rayCaster: Ammo.btDefaultVehicleRaycaster;
    ...
    // первые 3 знаечния позиция x,y,z, остальные 4 вращение (quaternion)
    // 0-6 - автомобиль,
    // 7-13 - переднее левое колесо
    // 14-20 - переднее правое колесо
    // 21-27 - заднее левое колесо
    // 28-34 - заднее правое колесо
    // 35 - скорость
    ...
    public update(dt: number): Float32Array<ArrayBufferLike> {
        ...
        if (this.actions.break && rs > 0) {
            ...
            if (this.actions.acceleration) {
                if (speed < -1) {
                    ...
                    // torque = -0.000003 * (this.rpm * this.rpm) + 0.04 * this.rpm + 300;
                    // this.engineForce = torque / 18;
        ...
        if (this.actions.left) {
            ...
            } else if (this.engineForce !== 0 && rs !== 0) {
        ...
        // Если скорость равна нулю и мы не нажали газ отправлять только вращение колес
        if (rs === 0 && this.engineForce === 0) {
        ...
        if (
            (this.actions.acceleration && rs > this.data.maxAccelerationSpeed)
            || (this.actions.braking && rs >= this.data.maxBrakingSpeed)
        ) {
            this.engineForce = 0;
        }
    ...
    private updateWheelTransform(i: number): Float32Array<ArrayBufferLike> {
